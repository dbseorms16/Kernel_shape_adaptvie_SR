...
def main():
    ...
    prog.add_argument('--sigma_x', '-sx', type=float, default=2.0, help='sigma_x')
    prog.add_argument('--sigma_y', '-sy', type=float, default=0.4, help='sigma_y')
    ...
    for i in range(len(data_mode_l)):
        ...
        for subfolder_GT in subfolder_GT_l:
            ...
            if 'LR' in subfolder_GT:
                continue
            print(subfolder_GT)
            # fname = int(subfolder_GT.split('\\')[-1].split('/')[0])
            # if fname > 14:
            #     continue
            ...
            if degradation_mode == 'preset':
                for index in range(seq_length):
                    ...
                    imgs_LR = imgs_HR.mul(255).clamp(0, 255).round().div(255)
                    imgs_LR_np = imgs_LR.permute(0, 2, 3, 1).cpu().numpy()
                    imgs_LR_np = (imgs_LR_np * 255).astype('uint8')
                    # for i, img_LR in enumerate(imgs_LR_np):
                    #     print(osp.join(save_subfolder, 'img{}.png'.format(i)))
                    imageio.imwrite(osp.join(save_subfolder, '{}.png'.format(osp.splitext(osp.basename(img_GT_path_l[index]))[0])), imgs_LR_np[0])
                    ...
                    imageio.imwrite(osp.join(save_subfolder2, '{}.png'.format(osp.splitext(osp.basename(img_GT_path_l[index]))[0])), imgs_SuperLR_np[0])
